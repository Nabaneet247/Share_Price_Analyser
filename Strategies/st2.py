...
DIRECTORY_PATH = BACKTEST_DATA_FOLDER + 'Strategy 2\\'
...
def add_indicators(share):
    ...
    add_50_day_low(share)
    add_100_day_low(share)
    add_200_day_low(share)
...
def get_low_days_value(low_type):
    return {LOW_50D: 50, LOW_100D: 100, LOW_200D: 200}[low_type]


@logger.catch
def back_test_strategy_for_a_stock(symbol, starting_capital, target_profit_percent, absolute_stop_loss_percent,
                                   trailing_sl_type, high_type, low_type, window_size, shares_data):
    ...
    entry_price = 0
    units_purchased = 0
    max_buffer = max(get_high_days_value(high_type), get_low_days_value(low_type))
    df = share.price_data.ohlcv[max_buffer + window_size:]
    wins = 0
    losses = 0
    ...
    for index in df.index.values:
        if entered:
            ...
            price = entry_condition(df, index, high_type, low_type, window_size)
            if price > 0:
                entered = True
                entry_price = price
...
def run_strategy_for_multiple_stocks(param):
    ...
    low_type = param['low_type']
    window_size = param['window_size']
    shares_data = param['shares_data']

    if get_high_days_value(high_type) > get_low_days_value(low_type):
        return

    individual_report_file_name = DIRECTORY_PATH + 'Individual Report - tp{} asl{} tsl{} ht{} lt{} ws{} sc{}.xlsx'.format(
        target_profit_percent, absolute_stop_loss_percent, trailing_sl_type[:2], get_high_days_value(high_type),
        get_low_days_value(low_type), window_size,
        starting_capital)
    ...
    try:
        ...
        individual_report = run_strategy_for_individual_stocks(starting_capital, target_profit_percent,
                                                               absolute_stop_loss_percent, trailing_sl_type,
                                                               high_type, low_type, window_size, shares_data)
    ...
    for case in cases:
        ...
        for max_positions in positions:
            report_number = 'tp{} asl{} tsl{} ht{} lt{} ws{} sc{} p{} c{}'.format(target_profit_percent,
                                                                                  absolute_stop_loss_percent,
                                                                                  trailing_sl_type[:2],
                                                                                  get_high_days_value(high_type),
                                                                                  get_low_days_value(low_type),
                                                                                  window_size,
            ...
            #    high_type, low_type, window_size)
            ...
            try:
                trade_log, daily_log, stats_log = back_test_for_multiple_shares(symbols, target_profit_percent,
                                                                                ...
                                                                                trailing_sl_type, high_type, low_type,
                                                                                window_size, shares_data)
...
def back_test_for_multiple_shares(symbols, target_profit_percent, absolute_stop_loss_percent, starting_capital,
                                  max_positions, trailing_sl_type, high_type, low_type, window_size, shares_data):
    ...
    max_buffer = max(get_high_days_value(high_type), get_low_days_value(low_type))
    for share in shares:
        share.price_data.ohlcv = share.price_data.ohlcv.iloc[max_buffer + window_size:]
    ...
    for date in dates:
        ...
        for share in shares:
            ...
            if share.symbol in portfolio.keys():
                ...
                price = entry_condition(df, date, high_type, low_type, window_size)
                if price > 0:
                    entry_price = price
    ...
    stats_log = pd.DataFrame(
        {'Timestamp': datetime.now(), 'Total Trades': trades_entered + trades_exited,
         ...
         'Trailing Stop Loss': trailing_sl_type, 'High Type': high_type, 'Low Type': low_type,
